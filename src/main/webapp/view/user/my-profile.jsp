            ...
            <div class="content-wrapper">
                <div class="user d-flex justify-content-between">
                    <div class="d-flex flex-column flex-md-row">
                        <div class="user-image me-4"
                             onclick="location.href='<%= request.getContextPath() %>/user/upload-picture'; ">
                            <c:if test="${profile.profilePicture == null}">
                                <img src="/images/user/user-default.png" alt="${profile.firstName}"/>
                            </c:if>
                            <c:if test="${profile.profilePicture != null}">
                                <img src="data:${profile.profilePicture.fileType};base64,${profile.profilePicture.image}"
                    ...
                    <div class="pt-4">
                        <a href="<%= request.getContextPath() %>/user/edit-profile">
                            <button class="btn btn-outline-secondary">Editeaza profilul</button>
                        </a>
                    </div>
                </div>
                <!-- About -->
                <c:if test="${profile.about != null}">
                    <div class="about mt-3">
                        <h5 class="fw-semibold">Despre</h5>
                        <p>${profile.about}</p>
                    </div>
                </c:if>
            </div>
